...
Navigate(`/admin/orders/basic/single/`)
   }
  /**
 ...
 */
  ...
  const rows = sortedData.map((row) => (
    <tr key={row.id}>
      ...
      <td>{row.commoditType.commodity.commodityName }</td>
      <td>{row.commoditType.commodityTypeName}</td>
      <td>{row.qty }</td>
      ...
      <td className='flex gap-x-2' >
        <Link to={`/admin/orders/basic/single/${row.id}`} > 
